...
export default function Activities() {
  const [activeCategory, setActiveCategory] = useState('museums');
  const [selectedMuseumSubcategory, setSelectedMuseumSubcategory] = useState('all');

  const categories = [
    { id: 'museums', label: '🏛️ Top 16 Museums & Tickets' },
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-[#15151D] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        ...
        {/* Category Pills - Multi-line Wrap for easy mobile access */}
      ...
      {/* SECTION 0: MUSEUMS (Top 16 Curated with Dual Action Buttons) */}
        ...
        <div className="space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h3 className="text-xl font-black text-white">Top 16 Berlin Museums & Direct Access</h3>
                <p className="text-xs text-gray-400">Official ticket portals on the left, Google Maps directions on the right.</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredMuseums.map((m) => (
              ...
              >
                ...
                {/* Dual Action Buttons: Left = Official Ticket, Right = Google Maps Directions */}
                <div className="pt-4 mt-4 border-t border-white/5 grid grid-cols-2 gap-2">
                  <a
                    href={m.ticketUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center justify-center space-x-1.5 py-2 px-2.5 rounded-xl bg-amber-400/15 hover:bg-amber-400/25 border border-amber-400/40 text-amber-300 hover:text-amber-200 text-xs font-bold transition-all cursor-pointer text-center"
                    title="Official museum ticket reservation & admission"
                  >
                    <Ticket className="w-3.5 h-3.5 flex-shrink-0" />
                    <span className="truncate">{m.isFree ? 'Free Info' : 'Buy Tickets'}</span>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center justify-center space-x-1.5 py-2 px-2.5 rounded-xl bg-amber-400 hover:bg-amber-300 text-black text-xs font-bold transition-all shadow-md cursor-pointer text-center"
                    title="Open exact address & navigation in Google Maps"
                  >
                    <MapPin className="w-3.5 h-3.5 flex-shrink-0" />
                    <span className="truncate">Directions</span>
                    <ExternalLink className="w-3 h-3 ml-0.5 opacity-70 flex-shrink-0" />
                  </a>
                </div>
              </div>
            ))}
